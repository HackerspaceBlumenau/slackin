  ...
  app.get('/', (req, res) => {
    ...
    let page = dom('html',
      dom('head',
        ...
        dom("script src=https://www.google.com/recaptcha/api.js?hl=pt-BR"),
  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    if (channels) {
      ...
      if (!channels.includes(channel)) {
        return res
        .status(400)
        .json({ msg: 'Canal não permitido' })
      }
      chanId = slack.getChannelId(channel)
      if (!chanId) {
        return res
        .status(400)
        .json({ msg: `Canal "${channel}" não encontrado` })
      }
    }
    ...
    if (emails && emails.indexOf(email) === -1) {
      return res
      .status(400)
      .json({ msg: 'E-mail não permitido' })
    }

    if (coc && '1' != req.body.coc) {
      return res
      .status(400)
      .json({ msg: 'É necessário concordar com o Código de Conduta' })
    }
      ...
      if (err) {
        ...
        if(resp.body.success){
          ...
          if (err) {
            return res
            .status(400)
            .send({ msg: "Verificação do Captcha falhou" });
          }
        }
