...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe, gcaptcha_sitekey }){
  let div = dom('.splash',
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' usuários online de ',
          dom('b.total', total), ' registrados.'
        ]
        : [dom('b.total', total), ' usuários já estão registrados.']
    ),
    dom('form id=invite',
      channels && (
        channels.length > 1
      ...
      dom('input.form-item type=email name=email placeholder=hsbnu@gmail.com '
        + (!iframe ? 'autofocus' : '')),
      dom('br'),
    ...
    dom('p.signin',
      'ou ',
      dom(`a href=https://${org}.slack.com target=_top`, 'faça login'),
      '.'
    ),
    !iframe && dom('footer',
      'powered by ',
